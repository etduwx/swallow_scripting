...
.linkset nOS_requestAction.nstackwords, 1
...
nOS_requestAction:
	...
	ecallf r11 
    # now do the channel communication. First setting the destination
    setd   res[r11], r0
    outct  res[r11], 0x3
    out    res[r11], r11  # send my address
    chkct  res[r11], 0x1
    # send the action
    out    res[r11], r1
    out    res[r11], r2
...
.linkset nOS_listenForAction.maxchanends, 0
...
# protocol for nOS control activity
# send end; wait for end to come back. then send four words of control information. Receive end, send end, done
# listenForAction has chanend in r0, stacks in r1, chanMap in r2
nOS_listenForAction:
	entsp  5
    ...
    stw r11, sp[4]
    outct  res[r0], 0x1
    add    r11, r0, 0  # free r0
    in     r0, res[r11]
    in     r1, res[r11]
    in     r2, res[r11]
    in     r3, res[r11]

	# hold channel open on purpose, whilst OS processes action

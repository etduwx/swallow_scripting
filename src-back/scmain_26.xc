...
void __initLinks()
{
  ...
  c = getChanend(swallow_cvt_chanend(0x190202));
  asm("ecallf %0"::"r"(c));
  c = getChanend(swallow_cvt_chanend(0x1b0102));
  asm("ecallf %0"::"r"(c));
...
#if defined(AEC_FORCE_26) || defined(AEC_FORCE)
#define AEC_VAL 0x10
#else
#define AEC_VAL 0x30
#endif
...
//Enable dynamic AEC if we want AEC enabled even on cores that are in use
...
#if AEC_DIVIDER_26 > 1
  write_pswitch_reg(swallow_id(26),XS1_PSWITCH_PLL_CLK_DIVIDER_NUM,AEC_DIVIDER_26 - 1);
  ctrl_data = getps(XS1_PS_XCORE_CTRL0);
  ctrl_data &= 0xffffffcf;
  ctrl_data |= AEC_VAL;
  setps(XS1_PS_XCORE_CTRL0, ctrl_data);
#elif AEC_DIVIDER > 1
  write_pswitch_reg(swallow_id(26),XS1_PSWITCH_PLL_CLK_DIVIDER_NUM,AEC_DIVIDER - 1);
  ctrl_data = getps(XS1_PS_XCORE_CTRL0);
  ctrl_data &= 0xffffffcf;
  ctrl_data |= AEC_VAL;
  setps(XS1_PS_XCORE_CTRL0, ctrl_data);
  ...
  par
  {
    nOS_start(swallow_cvt_chanend(0x001a0102),swallow_cvt_chanend(0x001a0202),0);

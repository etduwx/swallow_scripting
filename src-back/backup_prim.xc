...
client_createThread(0,100,i,core_list_prim[i]);
      ...
      for(unsigned i=0;i<NUM_CHILDREN_PRIM;i++){
	      ...
	      //channelListen(myChannels[i+NUM_CHILDREN_PRIM]);
      }
     ...
     /* for(unsigned i=0;i<NUM_CHILDREN_PRIM;i++){
	      child_token = channelReceiveWord(myChannels[i+NUM_CHILDREN_PRIM]);
	      if(child_token != 5) asm("ecallt r0");
      } 
      for(unsigned i=0;i<NUM_CHILDREN_PRIM;i++){
	      //	printOne(i+10);
	      channelSendWord(myChannels[i+NUM_CHILDREN_PRIM],1);
	      child_token = channelReceiveWord(myChannels[i+NUM_CHILDREN_PRIM]);
	      if(child_token != 1) asm("ecallt r0");
      }	
      for(unsigned i=0;i<NUM_CHILDREN_PRIM;i++){
	      channelSendWord(myChannels[i+NUM_CHILDREN_PRIM],2);
	      child_token = channelReceiveWord(myChannels[i+NUM_CHILDREN_PRIM]);
      }
...
//      client_createThread(1,100,0,16);
      ...
      control_channel <: (char) 3;
      control_channel <: 1024;
      ...
      if(foo==42){
		...
		printer[1] = V_T/sampleCount;
		printer[2] = I_T/sampleCount;
		printer[3] = V_MT/sampleCount;
		printer[4] = I_MT/sampleCount;
		printer[5] = V_MB/sampleCount;
		printer[6] = I_MB/sampleCount;
		printer[7] = V_B/sampleCount;
		printer[8] = I_B/sampleCount;
		printer[9] = V_IO/sampleCount;
		printer[10] = I_IO/sampleCount;
		printer[11] = V_DRAM/sampleCount;
		printer[12] = I_DRAM/sampleCount;

	//	printMany(13,printer);
/*
...
void xc_getCompletedSignal(chanend child_channels[]){
	...
	select{
		case(unsigned i=0;i<NUM_CHILDREN_PRIM;i++)

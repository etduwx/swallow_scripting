...
void channelSendWord(channel c, unsigned value)
{
	...
	static unsigned channelmaps[7][31];
	unsigned channel_id,dst;
	unsigned myThread, myID;	
	unsigned printer[4];
	...
	first = 0;
		...
		channel_id = (c << 16) >> 24;
	myThread = get_logical_core_id();
	myID = c >> 16;
	
	 	asm("getd %0,res[%1]":"=r"(dst):"r"(c));	
	if(channelmaps[myThread-1][channel_id] == 0){
	
	channelmaps[myThread-1][channel_id] = (dst >> 8) << 12; 
	
}	
	else channelmaps[myThread-1][channel_id] += 1;
	

	printer[0] = get_local_tile_id();
	printer[1] = myThread;
	printer[2] = 0xbeadbeef;
	printer[3] = dst;
	printMany(4,printer);	
...
{
	static unsigned fir = 1;
	static unsigned channel_maps[7][31];
	unsigned channel_id,dst;
	unsigned myThread, myID;	
	unsigned printer[4];
	unsigned value ;
	...
	myID = c >> 16;
	 	asm("getd %0,res[%1]":"=r"(dst):"r"(c));	
	if(channel_maps[myThread-1][channel_id] == 0){
		channel_maps[myThread-1][channel_id] = (dst >> 8) << 12; 
}
	else channel_maps[myThread-1][channel_id] += 1;

	printer[0] = get_local_tile_id();
	printer[1] = myThread;
	printer[2] = 0xfeedbeef;
	printer[3] = c;
